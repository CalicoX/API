...
- **AI 整段 Duo 展开（2026-09-11 Park）**：进屏顶铰链立正、底糊；入场透视要够（`rotateX` 最大 78、perspective 920、带 translateZ）。离开是**底边开始折**（`origin 50% 100%` + `rotateX+`），糊贴顶，不是原路倒回、也不是顶铰链再折。中间直立停约 0.45 屏（段高 145vh sticky），然后离场跟页面往下走，不要长锁。≤640 / reduce 定格。
