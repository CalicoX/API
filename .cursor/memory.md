...
- Tracking data 卡（2026-08-18 按 Park 参考重做）：浅灰井 `#eef1f5`，不要深色、不要点阵。构图跟 File Transfer 参考一样——中心紫药丸 `Tracking`（`#c9a6ff → #7c4dff`，软紫光），4 个白圆钮 + 2 条白芯片（In Transit / Alert），虚线连到药丸，5 圈白涟漪。图标用官方 SVG 灰色 mask（`#8b95a5`），不要彩色方砖。hover 整卡触发，圆钮/芯片只水平 nudge 3.2s，虚线 dash 走。reduce-motion 关。Expired / Undelivered / Not Found 这 3 态先不进图，避免挤掉留白。
...
- 2026-08-14 先停在这里。
- Tracking data：已改成浅底 File Transfer 构图。Park 再看颜色/疏密。
