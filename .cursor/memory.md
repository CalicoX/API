...
- Dashboard 卡（2026-08-21）：对照真实后台三块，**两卡上下叠、整体收窄**（Park：不要拉满井宽；`min(72%, 336px)`）。① **按物流主状态**：donut 在左（约 152px）、9 态图例单列在右；**扇区用 fill path**，禁止 circle stroke-dasharray。圆心空。绿 Delivered ~55% / 灰 Not Found 22%。② **近90天【查询不到】** 折线必须撑满卡内宽：`preserveAspectRatio="none"`，plot `l:24 / r:248` 贴边，不要两侧留空。浅蓝 `#5b9cf5`，tooltip「08-15 / Qty 0」。③ 井底 **最近消耗** 紫面积。Webhook toast 浮在井右下。hover 3.2s：扇区淡入、折线重画、曲线呼吸、toast 横滑。reduce-motion 全停。
