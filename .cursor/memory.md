...
- Tracking data 卡（2026-08-18）：浅灰井 `#eef1f5`。中心 17 logo。9 态纯色圆公转。涟漪是 6 层半透明白圆叠层，**不要描边**，每一层一点点淡灰蓝阴影（`rgba(148,163,184)`）塑形。个别状态（In Transit / Delivered / Alert）白胶囊包名称，里面仍是官方色圆。状态圆不要渐变。
