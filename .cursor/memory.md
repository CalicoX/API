...
- Visibility 卡（2026-08-21）：去掉 Auto-identified 下卡。故事：idle 只有 Add Number（单号空、Auto-detect）；**整卡 hover** ① 打字 LV123242CN ② Carrier 蓝盒扫光（现款 sweep，只播一次）再换成 DHL Express ③ 下方滑出 How-it-works 同款轨迹面板（Delivered / Time Info / Shipping Events）。leave 重置。reduce-motion 直接终态。
