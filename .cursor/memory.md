...
- Dashboard 卡（2026-08-21）：两卡上下叠、收窄 `min(72%, 336px)`，整体靠下。Webhook toast 锚在下卡右侧。① 状态 donut：fill path 扇区。② 90 天线：idle 只露贴零平段（`stroke-dashoffset: var(--rest)`）；hover 用 **dashoffset 100→0 从左往右画出**，禁止 dasharray 两值对插值。③ 井底紫面积：hover 用 `clip-path: inset(0 100% 0 0) → inset(0 0)` **从左往右擦出**。3.2s。reduce-motion 终态。
