...
- 承运商卡：底排 5 个淡出 logo 去掉，只留上两排（3+5）。井是浅灰 `#f7f8fa`，`::before` 点阵关掉，不要蓝渐变。背景是第二屏 Use Cases 同款 WebGL 点状地球（公共模块 `src/fx/lib/particle-earth.js`），不是 SVG 点、不是井点阵。地球要能读出半球：`scale: 1.3`，`offset [0,-1.05]`（赤道贴井底，下半被 overflow 裁掉，两侧略出血）。2026-08-21 Park 两轮都嫌太亮：`size: 1.55`，`alphaMul: 0.32`，大陆 `[0.16,0.3,0.52]` / Hi `[0.22,0.38,0.62]`，海洋 `[0.2,0.3,0.44]`，必须是蓝（白点在浅灰井上会看不见）。叠点会发亮，别再把 alpha 拉回 0.7。自转是 hover 驱动：平时定格零重绘（引擎 `getAnimating` 返回 false 时不排 rAF，`onApi.wake` 唤醒），整卡 `.api-s4-card` mouseenter 缓动加速到 0.18 rad/s、mouseleave 缓动刹停（速度因子指数逼近，dt 钳 0.1s）；reduce-motion / 触屏（`hover: none`）不挂监听、保持静态。不要用 absolute+aspect-ratio 空槽当 WebGL host（高度容易是 0，地球就没了）。不要铺成 124% 全幅。z-index 低于 3400+ / logo；横向 sticky + `overflow-x: clip` 时不要只靠 IntersectionObserver 才 start，用 getBoundingClientRect 判可见。卡滚出可视区停画。3400+ 视觉写成 `3,400+`（+ 略小、偏蓝），字重 700、tabular，carriers 大写字距。枢轴白卡约 132px 宽。logo 构图铺满井，地球不铺满。第一排 logo 在 y=232、第二排 y=324（2026-08-21 从 212/300 再下移）。`hubWirePath` 自动跟随，但 `stroke-dasharray`/keyframes 要 ≥340 盖住最长连线，否则线中间断。连线要淡：`rgba(140,165,205,0.22)` / `stroke-width: 1.15`，不要深灰蓝。白卡 **不要外边框 / 不要 6px 白晕**：1px 亮顶 `#fff` + 1px 暗底 `rgba(15,23,42,0.16)` 做体积，外加很淡投影。白卡顶部三层机架 SVG（76×46）：不要统一描边；每层 fill `#f4f7fb`，上沿 1px `#fff`、下沿 1px `#c3cedd`，蓝 LED + 浅蓝通风条/活动条。hover 时三颗蓝 LED 错峰闪（`api-s4-led`，delay `--i`×0.38s），reduce-motion 关。服务器在数字上方，卡 padding 收成 15/14。承运商 logo hover **逐个出现**（2026-08-20）：idle 全可见；整卡 hover 时 `animation-fill-mode: both` 先收成 0，再按 `--i` 每 0.12s 弹入（0.55s，`scale(0.78)→1`，位移只做居中 translate，不斜移）。连线同步 `wire-in`。只播一次不循环。reduce-motion 关动画、logo 保持居中 translate。
