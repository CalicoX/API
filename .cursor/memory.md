...
- Visibility 卡（2026-08-21）：去掉 Auto-identified 下卡。故事：静态终态（单号满、DHL、轨迹面板都在）；**整卡 hover** 才从头播 ① 打字 LV123242CN ② Carrier 蓝盒扫光（现款 sweep，只播一次）再换成 DHL Express ③ 下方滑出 How-it-works 同款轨迹面板（Delivered / Time Info / Shipping Events）。leave 回到终态。reduce-motion 不播、保持终态。
