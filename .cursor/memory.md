...
- Tracking data 井：深色，只比 `#0a0d14` 稍浅一档，不要改浅灰、不要改成中灰蓝 `#3c465e`。渐变中心 `#1c2434` → 边 `#111620`，再加很淡的白蓝径向。点阵用淡白点。承运商井是蓝色点状地球；Visibility / Dashboard 仍浅灰点阵。
- 承运商卡：底排 5 个淡出 logo 去掉，只留上两排（3+5）。井是蓝色（`#4a86ff` → `#1b57e0` → `#143eb4`），不要浅灰点阵。背景是点状地球（Fibonacci 球面投影），不是井点阵。3400+ 视觉写成 `3,400+`（+ 略小、偏蓝），字重 700、tabular，carriers 大写字距。枢轴白卡约 132px 宽。插图尽量铺满井。
