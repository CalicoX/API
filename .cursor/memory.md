...
- **pricing Popular 徽标（2026-08-31 Park，commit 9c4c438 + 926d319 + 00dc078）**：原实现是 `.api-plan-card { grid-template-rows: 36px 1fr }` 占位行（Flagship 蓝带在框内）——已删，四卡统一 flex column。徽标终态：`position:absolute; top:-38px; translateX(-50%)` **完全浮在框顶上方 8px**（独立块、不压框线），`.api-plans` 顶部留 38px 悬浮位。**卡 `overflow: hidden` 已放开成 visible**——别给 plan 卡加回。四卡框顶/标题对齐保持。非 hot 卡的 span 是 `\u00a0` 透明占位。390 tab 档只有 `is-active` 卡渲染，徽标随 Flagship 选中出现。**坑（00dc078）**：给 `.api-plans` 加 `padding-top` 时把 `padding: 0` 删了，ul 的 UA 默认 `padding-inline-start: 40px` 就放出来（390 卡左侧空 40px）——列表类容器的归零要显式写 `padding: 38px 0 0`，别只写单边。
