...
- **AI 整段 Duo 展开（2026-09-11 Park）**：进屏顶铰链立正、底糊；入场透视要够（`rotateX` 最大 78、perspective 920、带 translateZ）。离开是**底边开始折**（`origin 50% 100%` + `rotateX+`），糊贴顶，不是原路倒回、也不是顶铰链再折。离场**不要 sticky 锁屏**，段高 100vh，折页跟着模块往下走。≤640 / reduce 定格。
