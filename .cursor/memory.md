...
- Dashboard 卡（2026-08-21）：两卡上下叠、收窄 `min(72%, 336px)`，**整体靠下**（padding-top ~18%）。Webhook toast **锚在下卡右下并上移**（`right:-16px; bottom:28px`），不要贴井底。① 状态 donut：fill path 扇区。② 90 天线：idle **只画贴零平段**，后半拉升/平台 **hover 才画出**（Park 框的那截不能一开始就在）；`preserveAspectRatio="none"` 撑满卡宽。③ 井底紫面积。hover 3.2s。reduce-motion 折线直接终态。
