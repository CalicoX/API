...
- Dashboard 卡（2026-08-21）：对照真实后台三块，**两卡通栏叠放**（Park 嫌并排太小）。① **按物流主状态**：donut 在左（约 152px）、9 态图例单列在右；**扇区用 fill path**，禁止 circle stroke-dasharray（1–2% 的红/橙会变成径向折线）。圆心空。绿 Delivered ~55% / 灰 Not Found 22%。② **近90天【查询不到】** 通栏折线：08-14～08-18 贴 0、08-19 抬到 2100 再平台，浅蓝 `#5b9cf5`，tooltip「08-15 / Qty 0」。③ 井底 **最近消耗** 紫面积。Webhook toast 在两卡下方。hover 3.2s：扇区淡入、折线重画、曲线呼吸、toast 横滑。reduce-motion 全停。
