...
- Explore 17RETURNS 右侧跟 tracking 同一套（2026-09-23）：Returns hero 整组插图的终态，等比缩小。底图是 returns 仓 `hero-returns.jpg`（覆盖 `returns-kv.jpg`，864×1152）。设计盒 704×572，`--kv-s: min(0.5, 100cqw / 704px)`，`.returns-ui` 是 `container-type: inline-size` + `overflow: clip`。**不要写死 scale**（0.86 在 390 撑出视口，Park 截过）。停在最后一帧，无动画、无 hover。三张卡直接亮着。旧的「三张卡各自 scale(0.64) + 88px 连接线」已废。
- 用户：Park，设计师，直接改代码迭代
...
- **五连改（2026-09-21 Park 批量圈点，一 commit）**：
  ...
  5. **Applications 段下线，内容并入 Use Cases**：LandingPage 不再挂 Applications（组件文件留存）；五张卡变成 Use Cases 标题下 `.api-s2-app-list` 五条（icon + 标题 + 描述，**无卡片框**）。**出现节奏（2026-09-21 Park 两轮定案）**：全部一起留在屏上，出现依次错开——`use-cases-scroll.js` apply() 里第 i 条阈值 `0.1 + i*0.16`（p 过线加 `.is-in`，CSS `opacity+translateY` 过渡；先做过「只亮当前条」被否）。**布局**：`.api-s2-copy` 从 `space-between`（列表钉底、和标题隔一大段）改 `flex-start`，列表 max-width 380、距标题 48px（Park「拉开点距离」从 26 加到 48）；1990 行附近旧「data-uc-line 全亮」规则已删。**手机档取消 sticky（同日 Park「怎么只有四个」）**：≤640 堆叠后标题+五条高于 100vh 舞台，`.api-s2-sticky` 的 overflow:hidden 把第 5 条裁掉——≤640 块改 `position: relative; top/height auto !important`（JS layout() 写 inline，必须 !important）+ `.api-s2-scroll` height auto，整段自然流，p 定格 1 五条全亮。**左侧 3D 井缩两档**：`.api-s2-visual` height 100%→86%→74%→**80%**（Park 三轮：缩小/再缩小/「稍微大一点点」）、`align-self: center` + `margin-left: -2.5%`（整体左移，相机随 host 自动重排）。**callout 导引线收短（同日 Park「缩缩短」）**：`iso-hub-webgl.js` 的 `buildCalloutGeometry` 加 `inset` 参数——药丸标签从画布边缘往场景方向收（Register/Webhook 120、Track 170），线长随 pill 位置自动缩短。右下三条旧 bullet（Track and trace…）删除。Applications 的 Point Waves 挂载一并摘除。structure.test 全部同步（段落序、app-list 断言、hero 渐变/globe 断言），20/20 过。
  注意：AiIntelligence 的 `progressive-blur` 字样在 CSS 里已不存在（只剩 `.api-ai-pblur` 规则），structure.test 174 行断言已改查 `api-ai-pblur`。

- **Explore Tracking 插图换 returns 版（2026-09-21 Park）**：API ExploreMore 的 tracking 插图从旧 `.track-ui-board`（icon 进度 + Shipping Events 轨道 + Brand video）整体换成 returns（5173）同款：`.os-status`（"Your order has been delivered." + 纯圆点紫进度线 + Order pending…Delivered 标签 + USPS 9400 单号行 + `.os-events` 30 条事件流，首条绿字 is-latest，底部 mask 渐隐）+ `.float-card-metric` WISMO 浮卡（−12% 标签 + ↓95% + mini-bars，右缘 right:-8px 压边）。Brand video 浮卡删除（returns 侧也删了）。**CSS 全部移植到 api-page.css 末尾，命名空间 `.api-explore`**（os-* 全新类不冲突；`.float-card` 基础样式一并搬过来，API landing.css 没有）；TRACK_EVENTS 数据内联在 ExploreMore.jsx。**坑：landing.css ≤640 旧 PC 缩放块带 `transform: scale(0.87) !important` + 354×435 定宽高——中和必须也 `!important`**（api-page.css 同特异性也赢不了 !important）。**坑二：landing.css ≤768 有 `.float-card { display:none !important }`（tracking 站手机藏浮卡旧规）→ 手机上 WISMO 消失；修法照 returns 118622f：api-page.css ≤768 块对 `.float-card-metric` 写 `display/position !important` 接住，不要去改 landing.css**。响应式照搬 returns：1024 收浮卡 / 768 收井距 / 640 满宽 hidden + os-status 6px。文案侧（ORDER TRACKING 标题 + 描述 + Explore CTA）不动。
...
- 2026-09-11 16:13 — memory.md、ai-duo-unfold.js、api-page.css
- 2026-09-11 16:12 — memory.md、ai-duo-unfold.js、api-page.css
- 2026-09-11 16:11 — memory.md、ai-duo-unfold.js、api-page.css
- 2026-09-11 16:06 — memory.md、ai-duo-unfold.js、api-page.css
