...
- Tracking data 卡（2026-08-18）：File Transfer 构图（浅灰井、白圆钮、白芯片、虚线、涟漪），但中心必须是 17 logo（超椭圆 64px，不要紫药丸「Tracking」字）。状态图标必须用官方色方砖（Info `#00bcd4`、Transit `#2196f3`、Out `#2962ff`、Pickup `#0d47a1`、Alert `#ff6f00`、Delivered `#43a047`），不要改灰。涟漪圈以 logo 为圆心（stage 正方形 `100cqmin`，left/top 50%），**不要描边**，只用半透明白填。hover 整卡水平 nudge 3.2s。Expired / Undelivered / Not Found 先不进图。
