...
- **footer 布局分档（2026-08-31 Park「1024 footer 不该变上下堆叠」，commit 56f198f）**：>768 = 桌面式左右（brand 左 max 480 + nav 右侧 2×2 两列）；≤768 才上下堆叠（堆叠规则从原 ≤1024 块挪进 10535 ≤768 块）。nav 的 2×2 两列规则留在 ≤1024（桌面 4 列并排在 976px 放不下）。
...
- **pricing Popular 徽标终态（2026-08-31 Park 四轮迭代，commit 73e9877）**：官网原版形态——**分离的圆角条完全悬在 Flagship 框外上方**（`absolute; top:auto; bottom:100%; left:0; right:0; height:46px; radius:12px; margin-bottom:6px`，蓝底白字），四卡框顶/标题对齐不受影响（absolute 不占流），`.api-plans` 顶部 `padding: 52px 0 0` 留悬浮位。废弃方案：框内 36px 占位行（初版）、压线胶囊、框上方 -38px 小胶囊、框内色带白体内嵌。**坑：绝对定位同时写 top 和 bottom 时 bottom 被忽略——覆盖基础规则的 top 必须 `top: auto`**（基础 `.api-plan-popular` 有 top:0）。**卡 `overflow: visible` 保持**。非 hot 卡 span 是 `\u00a0` 透明占位。390 tab 档徽标随 Flagship 选中出现。另：`.api-plans` 归零要显式四向（padding 单边写会放出 ul 的 UA 默认 padding-inline-start:40px）。
