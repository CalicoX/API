...
`LandingPage.jsx`：Hero → TrustBand → CoverageBand → Enterprise → UseCases → HowItWorks → DataOperations → AiIntelligence → Applications → IntegrationTogether → Credentials → ExploreMore → BottomCta。
