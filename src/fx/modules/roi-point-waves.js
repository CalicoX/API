/**
 * Applications 屏背景 — shaders.com「Point Waves 1」(569774bc) 自研复刻。
 * 从 Returns ROI（#returns-roi / .rt-roi-waves）原样接入，挂到 #applications。
 *
 * 原预设图层（payload 实锤）：
 *   SolidColor(#080808)
...
export function mount() {
  ...
  try {
    ...
    let canvas = section.querySelector(".api-s5-waves");
    if (!canvas) {
      canvas = document.createElement("canvas");
      canvas.className = "api-s5-waves";
