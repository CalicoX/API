/**
 * Undertones 1 — 1:1 port of the shaders.com preset (npm package `shaders` v3.0.453).
 *
 * Original pipeline (fixed four-layer recipe, verified against dist/core sources):
 *   Root → FilmGrain( FlutedGlass( over(ChromaFlow, Swirl) ) )
 *
 * Mapped to WebGL as two passes:
 *   Pass 1 (FBO):   Swirl gradient + ChromaFlow premultiplied-over composite
 *   Pass 2 (screen): FlutedGlass refraction + FilmGrain + linear→sRGB encode
 *
 * Faithfulness notes:
 *   - All colors are converted sRGB→linear on the CPU; every mix/add happens in
 *     linear light (the original engine renders to a `-srgb` surface).
 *   - ChromaFlow field: 128×128 RGBA16F texture (half-float, linear filtered),
 *     CPU advection/injection loop is a verbatim port of the original
 *     onBeforeRender (fade = 1-dt, radius = radius*0.05, speed²-scaled
 *     injection radius, momentum*50*dt advection).
 *   - FlutedGlass: rounded shape (expHi 8, expLo 3), exponent = mix(8,3,softness),
 *     fresnel = (1-nz)^5, fresnelMix = 0.04+0.96f, shininess = exp2(8-hs*7),
 *     mirror edge sampling, speed 0 → static flutes.
 *   - FilmGrain: static hash grain, Rec.709 luma, darkFactor = (1-luma)^bias,
...
export function mount() {
  ...
  try {
    ...
    let canvas = section.querySelector(".api-s1-shader");
    if (!canvas) {
    ...
    section.classList.add("has-hero-wash");
    ...
    // colorB measured from the live shaders.com render at rest (flat plateau #eb);
    // #f0f0f0 from the payload palette is a thumbnail average, not the prop value.
...
void main(){
  ...
  gl_FragColor = vec4(encodeSrgb(col), 1.0);
    ...
    teardown = () => {
      ...
      section.classList.remove("has-hero-wash");
    };
  } catch (err) {
