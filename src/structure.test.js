...
describe("API landing structure (gating)", () => {
  ...
  it("CoverageBand replaces BrandsSay after TrustBand with globe + 5 metrics", () => {
    ...
    expect(band).toMatch(/3,500\+/);
    expect(band).toMatch(/230\+/);
    expect(band).toMatch(/33\+/);
    expect(band).toMatch(/22M\+/);
    expect(band).toMatch(/16 years/);
    expect(band).toMatch(/Countries and Regions/);
    expect(band).toMatch(/Daily Tracking Requests/);
    expect(band).toMatch(/Industry Expertise/);
